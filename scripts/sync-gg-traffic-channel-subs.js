/**
 * One-shot: set exact GG subs template on every GG traffic channel.
 *
 *   node scripts/sync-gg-traffic-channel-subs.js           # dry-run (default)
 *   node scripts/sync-gg-traffic-channel-subs.js --apply   # write to RedTrack
 */
require("dotenv").config();
const { GG_TRAFFIC_CHANNEL_SUBS } = require("../config/redtrackGgSubsTemplate");
...
(async () => {
  ...
  const ggChannels = sources.filter(
    (s) => redtrackService.getTrafficChannelPlatform(s.title) === "google"
  );

  console.log(`Found ${ggChannels.length} GG traffic channel(s)\n`);

  const summary = { total: ggChannels.length, skipped: 0, updated: 0, failed: 0 };
  const templateSubs = redtrackService.cloneSubsTemplate(GG_TRAFFIC_CHANNEL_SUBS);

  for (const channel of ggChannels) {
  ...
  if (!APPLY && summary.updated > 0) {
    ...
    console.log("  node scripts/sync-gg-traffic-channel-subs.js --apply");
  }
